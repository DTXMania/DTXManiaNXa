    ...
    public class CSoundManager   // CSound管理
    {
            ...
            bool _bUseOSTimer )
        {
            ...
            if( soundDeviceType == ESoundDeviceType.ExclusiveWASAPI || soundDeviceType == ESoundDeviceType.ASIO || soundDeviceType == ESoundDeviceType.SharedWASAPI )
            {
                ...
                Bass.Configure( Configuration.UpdateThreads, nCPUCores );
        ...
        public float GetCPUusage()
        {
            ...
            switch( SoundDeviceType )
            {
                case ESoundDeviceType.ExclusiveWASAPI:
                case ESoundDeviceType.SharedWASAPI:
                    f = (float) BassWasapi.CPUUsage;
                    break;
                case ESoundDeviceType.ASIO:
                    f = (float) BassAsio.CPUUsage;
                    break;
                case ESoundDeviceType.DirectSound:
                    f = 0.0f;
                    break;
    ...
    public class CSound : IDisposable, ICloneable
    {
            ...
            set
            {
                if( _db再生速度 != value )
                {
                    ...
                    if( bIsBASS )
                    {
                        ...
                        {
                            Bass.ChannelSetAttribute( this.hBassStream, ChannelAttribute.Tempo, (float) ( dbPlaySpeed * 100 - 100 ) );
        ...
        public int nVolume  // n音量
        {
            get
            {
                ...
                {
                    float f音量 = 0.0f;
                    if( !Bass.ChannelGetAttribute( this.hBassStream, ChannelAttribute.Volume, out f音量 ) )
            ...
            set
            {
                if( this.bIsBASS )
                {
                    ...
                    Bass.ChannelSetAttribute( this.hBassStream, ChannelAttribute.Volume, f音量 );
        ...
        public int nPosition  // n位置
        {
            ...
            set
            {
                ...
                {
                    // -100～100 → -1.0～1.0
                    float f位置 = Math.Min( Math.Max( value, -100 ), 100 ) / 100.0f;
        ...
        public void tDirectSoundサウンドを作成するXaOggMp3( string strファイル名, DirectSound DirectSound )
        {
            ...
            var wfx = WaveFormat.CreateCustomFormat( WaveFormatEncoding.Pcm, (int) cw32wfx.nSamplesPerSec, (int) cw32wfx.nChannels, (int) cw32wfx.nAvgBytesPerSec, (int) cw32wfx.nBlockAlign, (int) cw32wfx.wBitsPerSample );
        ...
        public bool b一時停止中
        {
            get
            {
                if( this.bIsBASS )
                {
                    bool ret = ( BassAux.BassMix_ChannelIsActive( this.hBassStream ) == BassActive.Paused ) &
        ...
        public bool b再生中
        {
            get
            {
                ...
                else
                {
                    ...
                    bool ret = ( BassAux.BassMix_ChannelIsActive( this.hBassStream ) == BassActive.Playing );
        ...
        public void tPlaySound( bool bループする )
        {
            if( this.bIsBASS )          // BASSサウンド時のループ処理は、t再生を開始する()側に実装。ここでは「bループする」は未使用。
            {
                ...
                bool b = BassAux.BASS_Mixer_ChannelPlay( this.hBassStream );
                if( !b )
                {
                    ...
                    bool bbb = BassAux.BASS_Mixer_ChannelPlay( this.hBassStream );
                    if( !bbb )
                    {
        ...
        public void tStopSound( bool pause )
        {
            if( this.bIsBASS )
            {
                ...
                BassAux.BASS_Mixer_ChannelPause( this.hBassStream );
                if( !pause )
                {
        ...
        public void tChangePlaybackPosition( long n位置ms )  // t再生位置を変更する
        {
            if( this.bIsBASS )
            {
                ...
                try
                {
                    b = BassMix.ChannelSetPosition( this.hBassStream, Bass.ChannelSeconds2Bytes( this.hBassStream, n位置ms * this.db周波数倍率 * this.dbPlaySpeed / 1000.0 ), PositionFlags.Bytes );
                }
                catch( Exception e )
                {
        ...
        private void tBASSサウンドを作成する( byte[] byArrWAVファイルイメージ, int hMixer, BassFlags flags )
        {
            ...
            _hBassStream = Bass.CreateStream( hGC.AddrOfPinnedObject(), 0, byArrWAVファイルイメージ.Length, flags );
            if( _hBassStream == 0 )
                throw new Exception( string.Format( "サウンドストリームの生成に失敗しました。(BASS_StreamCreateFile)[{0}]", Bass.LastError.ToString() ) );
        ...
        private void tBASSサウンドを作成するXA( string strファイル名, int hMixer, BassFlags flags )
        {
            ...
            _hBassStream = Bass.CreateStream( this.hGC.AddrOfPinnedObject(), 0L, totalPCMSize, flags );
            if( _hBassStream == 0 )
            {
                hGC.Free();
                throw new Exception( string.Format( "サウンドストリームの生成に失敗しました。(BASS_SampleCreate)[{0}]", Bass.LastError.ToString() ) );
            }
        ...
        private void tBASSサウンドを作成する_ストリーム生成後の共通処理( int hMixer )
        {
            ...
            {
                _hTempoStream = BassFx.TempoCreate( _hBassStream, BassFlags.Decode | BassFlags.FxFreeSource );
                if( _hTempoStream == 0 )
                {
                    hGC.Free();
                ...
                {
                    Bass.ChannelSetAttribute( _hTempoStream, ChannelAttribute.TempoUseQuickAlgorithm, 1f );    // 高速化(音の品質は少し落ちる)
                }
            }
...
#endif
            {
                var bf = BassFlags.SpeakerFront | BassFlags.MixerChanNoRampin | BassFlags.MixerChanPause;
