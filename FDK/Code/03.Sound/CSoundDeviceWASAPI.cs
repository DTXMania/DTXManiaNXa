...
namespace FDK
{
    public class CSoundDeviceWASAPI : ISoundDevice
    {
        ...
        public CSoundDeviceWASAPI( Eデバイスモード mode, long n希望バッファサイズms, long n更新間隔ms, string strRecordFileType, string strEncoderPath )
        {
            ...
            //BassNet.Registration( <メアド>, <パスコード> );
            #endregion
            ...
            var bassDevInfos = BassAux.BASS_GetDeviceInfos();
            for( a = 0; a < bassDevInfos.GetLength( 0 ); a++ )
            {
                {
                    Trace.TraceInformation( "Sound Device #{0}: {1}: IsDefault={2}, isEnabled={3}, id={4}",
                        a,
                        bassDevInfos[ a ].Name,
                    ...
                    if( bassDevInfos[ a ].IsDefault )
                    {
                        ...
                        string[] s = bassDevInfos[ a ].Driver.ToString().ToUpper().Split( new char[] { '#' } );
                        if( s != null && s[ 0 ] != null )
                        {
            ...
            WasapiDeviceInfo deviceInfo = default;
            int nDevNo = -1;
            for( int n = 0; n < BassWasapi.DeviceCount; n++)
            {
                deviceInfo = BassWasapi.GetDeviceInfo( n );
            ...
            if( nDevNo != -1 )
            {
                ...
                if( !Bass.Init( 0, deviceInfo.MixFrequency ) )  // device = 0:"no device": BASS からはデバイスへアクセスさせない。アクセスは BASSWASAPI アドオンから行う。
            ...
            //int a, count = 0;
            WasapiDeviceInfo wasapiDevInfo;
            for( a = 0; a < BassWasapi.DeviceCount; a++ )
            {
                wasapiDevInfo = BassWasapi.GetDeviceInfo( a );
            
                if( !wasapiDevInfo.IsInput    // device is an output device (not input)
                 && wasapiDevInfo.IsEnabled ) // and it is enabled
                {
                    Trace.TraceInformation( "WASAPI Device #{0}: {1}: IsDefault={2}, defPeriod={3}s, minperiod={4}s, mixchans={5}, mixfreq={6}",
                        a,
                        wasapiDevInfo.Name,
                        wasapiDevInfo.IsDefault, wasapiDevInfo.DefaultUpdatePeriod, wasapiDevInfo.MinimumUpdatePeriod, wasapiDevInfo.MixChannels, wasapiDevInfo.MixFrequency );
                }
            }
            #endregion
        ...
        //Retry:
            ...
            float fPeriod = (float) ( ( mode == Eデバイスモード.排他 ) ? deviceInfo.MinimumUpdatePeriod : deviceInfo.DefaultUpdatePeriod );
            ...
            if( mode == Eデバイスモード.排他 )
            {
                if( ( flags & WasapiInitFlags.EventDriven ) != WasapiInitFlags.EventDriven &&
                    f希望バッファサイズsec < f更新間隔sec * 4 )
                {
                    f希望バッファサイズsec = f更新間隔sec * 4;
                }
                else if( ( flags & WasapiInitFlags.EventDriven ) == WasapiInitFlags.EventDriven &&
            ...
            this.hMixer = BassMix.CreateMixerStream(
                info.Frequency,
                info.Channels,
                BassFlags.MixerNonStop | BassFlags.Float | BassFlags.Decode | BassFlags.MixerPositionEx );    // デコードのみ＝発声しない。WASAPIに出力されるだけ。
#endif
            if( this.hMixer == 0 )
            {
                var errcode = Bass.LastError;
            ...
            for( int i = 0; i <= (int) CSound.EInstType.Unknown; i++ )
            {
...
#else
                this.hMixer_Chips[ i ] = BassMix.CreateMixerStream(
                    info.Frequency,
                    info.Channels,
                    BassFlags.MixerNonStop | BassFlags.Float | BassFlags.Decode | BassFlags.MixerPositionEx );    // デコードのみ＝発声しない。WASAPIに出力されるだけ。
#endif
                ...
                // Mixerのボリューム設定
                Bass.ChannelSetAttribute( this.hMixer_Chips[ i ], ChannelAttribute.Volume, CSoundManager.nMixerVolume[ i ] / 100.0f );
...
#else
                bool b1 = BassMix.MixerAddChannel( this.hMixer, this.hMixer_Chips[ i ], BassFlags.Default );
#endif
                if( !b1 )
                {
                    var errcode = Bass.LastError;
        ...
        protected void Dispose( bool bManagedDispose )
        {
            ...
            if( this.hMixer_DeviceOut != 0 )
            {
                BassAux.BASS_Mixer_ChannelPause( this.hMixer_DeviceOut );
                Bass.StreamFree( this.hMixer_DeviceOut );
                this.hMixer_DeviceOut = 0;
            }
            if( this.hMixer_Record != 0 )
            {
                BassAux.BASS_Mixer_ChannelPause( this.hMixer_Record );
                Bass.StreamFree( this.hMixer_Record );
            ...
            if( this.hMixer_Chips != null )
            {
                for( int i = 0; i <= (int) CSound.EInstType.Unknown; i++ )
                {
                    if( this.hMixer_Chips[ i ] != 0 )
                    {
                        ...
                        BassAux.BASS_Mixer_ChannelPause( this.hMixer_Chips[ i ] );
                        Bass.StreamFree( this.hMixer_Chips[ i ] );
