...
namespace FDK
{
    public unsafe class Cmp3ogg : SoundDecoder
    {
        ...
        {
            bool r = Bass.Init( 0, 48000 );

            stream_in = Bass.CreateStream( filename, 0, 0, BassFlags.Default | BassFlags.Decode );
            ...
            nTotalPCMSize = Bass.ChannelGetLength( stream_in );

            #region [ Getting WAVEFORMEX info ]
            var chinfo = Bass.ChannelGetInfo( stream_in );
            wfx = new CWin32.WAVEFORMATEX(
                (ushort) 1,                                         // wFormatTag
                (ushort) chinfo.Channels,                           // nChannels
                (uint) chinfo.Frequency,                            // nSamplesPerSec
                (uint) ( chinfo.Frequency * 2 * chinfo.Channels ),  // nAvgBytesPerSec
                (ushort) ( 2 * chinfo.Channels ),                   // nBlockAlign
                16,                                                 // wBitsPerSample
                0                                                   // cbSize
            );
            #endregion
        ...
        public override int Decode( ref byte[] Dest, long offset )
        {
            ...
            do
            {
                len = Bass.ChannelGetData( stream_in, data, LEN );
                if( len < 0 )
                {
                    var be = Bass.LastError;
