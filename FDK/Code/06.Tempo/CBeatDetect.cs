...
namespace FDK
{
    public class CBeatDetect : IDisposable
    {
        ...
        private void Initialize()
        {
            ...
            if( !Bass.ChannelGetAttribute( this.hBassStream, ChannelAttribute.Frequency, out fFreq ) )
            {
        ...
        {
            fTempo = BassFx.BPMDecodeGet(
                this.hBassStream,
                0,
                nTotalSeconds,
                ( 300 << 16 ) + 70,     // MAX BPM=320, MIN BPM=70
                                        //0,
                BassFlags.Default,      //BASSFXBpm.BASS_FX_BPM_MULT2,
                null,
                IntPtr.Zero );
            return fTempo;
        }
        ...
        public List<stBeatPos> GetBeatPositions()
        {
            ...
            var _beatProc = new BPMBeatProcedure( GetBeat_ProgressCallback );

            bool ret = BassFx.BPMBeatDecodeGet(
                this.hBassStream,
                0,
                nTotalSeconds,
                //0,
        ...
        {
            BassFx.BPMFree( this.hBassStream );
            Bass.StreamFree( this.hBassStream );
            this.hBassStream = -1;
            Bass.Free();
        }
