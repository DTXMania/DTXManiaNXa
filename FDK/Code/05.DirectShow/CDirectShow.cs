        ...
        public unsafe void t現時点における最新のスナップイメージをTextureに転写する( CTexture texture )
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        // .NET Core 以降、BeginInvoke/EndInvoke は非推奨となったため、Task.Run() を使用する。（2025.11.8 FROM）

                        // 転送開始。
                        var ar = new Task[ CDirectShow.n並列度 ];
                        for( int i = 0; i < CDirectShow.n並列度; i++ )
                        {
                            ar[ i ] = ( bARGB32 ) ?
                                Task.Run(() => this.dgライン描画ARGB32[ i ].Invoke( i ) ) :
                                Task.Run(() => this.dgライン描画XRGB32[ i ].Invoke( i ) );
                        }
