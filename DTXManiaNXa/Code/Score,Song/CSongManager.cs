...
namespace DTXManiaNXa
{
	...
	internal class CSongManager
	{
        ...
        public AutoResetEvent AutoReset                     // 自動実装プロパティを使うとエラーになる。（自動生成されるprivateフィールドがシリアライズの対象になるため）
        {
            get => _AutioReset;
            private set => _AutioReset = value;
        }
        [NonSerialized]
        private AutoResetEvent _AutioReset;
...
//Trace.TraceInformation( "songs.db に存在しません。({0})", node.arScore[ lv ].FileInformation.AbsoluteFilePath );
        ...
        /// <summary>
        /// スキンが変更された場合、曲リストが持つパス（Preimageなど）にスキンパスが含まれているなら変更しなければならない。(2025.11.7; FROM)
        /// </summary>
        public void t現在のスキンをpreimageのパスに適用する()
        {
            this.t現在のスキンをpreimageのパスに適用する( this.listSongRoot );
        }
        private void t現在のスキンをpreimageのパスに適用する( List<CSongListNode> nodeList )
        {
            if( nodeList == null )
                return;

            // すべてのノードについて、スキンパスが含まれるフィールドを現在のスキンパスで上書きする。
            foreach( CSongListNode node in nodeList )
            {
                // RANDOM SELECT
                if( node.eNodeType == CSongListNode.ENodeType.RANDOM )
                {
                    for( int i = 0; i < 5; i++ )
                        node.arScore[ i ].SongInformation.Preimage = CSkin.Path( @"Graphics\5_preimage random.png" );
                }
                // BACK
                else if( node.eNodeType == CSongListNode.ENodeType.BACKBOX )
                {
                    node.arScore[ 0 ].SongInformation.Preimage = CSkin.Path( @"Graphics\5_preimage backbox.png" );
                }
                // 他にもスキンパスノードを持つノードが増えたら、適宜追加すること。

                // 子リストがあれば再帰する。
                this.t現在のスキンをpreimageのパスに適用する( node.list子リスト );
            }
        }
        //-----------------
        #endregion
...
Debug.WriteLine( dBPM + ":" + c曲リストノード.strタイトル );
        ...
        private List<string> listStrBoxDefSkinSubfolderFullName = new();
