...
//		public bool bBASS演奏で直前のBASSを消音する;
		...
		public void tLoadBMP_BMPTEX()
		{
			...
			if ( this.listBMP != null )
			{
				...
				else
				{
					...
					nLoadDone = 0;
                    //backgroundBMPLoadAll.BeginInvoke( listBMP, null, null ); --> .NET Core 1.0 以降ではサポートされない。
                    //var _ = Task.Run( () => backgroundBMPLoadAll.Invoke( listBMP ) ); --> listBMP が排他処理されていないので並列動作できない。
                    backgroundBMPLoadAll.Invoke( listBMP );
			...
			if ( this.listBMPTEX != null )
			{
				...
				else
				{
                    ...
                    //backgroundBMPTEXLoadAll.BeginInvoke( listBMPTEX, null, null );--> .NET Core 1.0 以降ではサポートされない。
                    //var _ = Task.Run( () => backgroundBMPTEXLoadAll.Invoke( listBMPTEX ) ); --> listBMPTEX が排他処理されていないので並列動作できない。
                    backgroundBMPTEXLoadAll.Invoke( listBMPTEX );   // 逐次処理。
        ...
        public void tドコドコ仕様変更(EInstrumentPart part, DTXManiaNXa.EType eDkdkType)
        {
            if ((part == EInstrumentPart.DRUMS) && (eDkdkType != DTXManiaNXa.EType.A))
            {
                if (eDkdkType == DTXManiaNXa.EType.B)
                {
                    int num = 0;
                    int index = 0;
                ...
                else if (eDkdkType == DTXManiaNXa.EType.C)
                {
                    int num11 = 0;
        ...
        public void t譜面仕様変更(EInstrumentPart part, DTXManiaNXa.EType eNumOfLanes)
        {
            if ((part == EInstrumentPart.DRUMS) && (eNumOfLanes != DTXManiaNXa.EType.A))
            {
                int num = 0;
                if (eNumOfLanes == DTXManiaNXa.EType.B)
                {
                    foreach (CChip chip in this.listChip)
                    {
                ...
                else if (eNumOfLanes == DTXManiaNXa.EType.C)
                {
                    bool flag = false;
                    bool flag2 = false;
                    EChannel num3 = 0;
        ...
        private static void tミラーチップのチャンネルを指定する( CChip chip, EChannel nミラー化前チャンネル番号 )
        {
            switch( nミラー化前チャンネル番号 )
            {
                case EChannel.HiHatClose:
                case EChannel.HiHatOpen:
                    if (CDTXMania.ConfigIni.eNumOfLanes.Drums != DTXManiaNXa.EType.B )
                    {
                        chip.nChannelNumber = ( (CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXManiaNXa.EType.A ) ? EChannel.RideCymbal : EChannel.Cymbal );
                        return;
                    }
                    break;
                case EChannel.Snare:
                    chip.nChannelNumber = ( (CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXManiaNXa.EType.C ) ? EChannel.LowTom : EChannel.FloorTom );
                    return;
                case EChannel.BassDrum:
                    if (CDTXMania.ConfigIni.eNumOfLanes.Drums != DTXManiaNXa.EType.C)
                    {
                        chip.nChannelNumber = EChannel.LeftBassDrum;
                        return;
                    }
                ...
                case EChannel.HighTom:
                    ...
                    break;
                case EChannel.LowTom:
                    chip.nChannelNumber = ( (CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXManiaNXa.EType.C ) ? EChannel.Snare : EChannel.HighTom);
                    return;
                case EChannel.Cymbal:
                    chip.nChannelNumber = ( (CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXManiaNXa.EType.C ) ? EChannel.HiHatClose : EChannel.LeftCymbal);
                    return;
                case EChannel.FloorTom:
                                    ...
                                    case ERandomMode.ANOTHERRANDOM:
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    if (CDTXMania.ConfigIni.eNumOfLanes.Drums == DTXManiaNXa.EType.A && (nチャンネル番号 == EChannel.Cymbal || nチャンネル番号 == EChannel.RideCymbal))
                                                    {
        ...
        public void tRandomizeDrumPedal(EInstrumentPart part, ERandomMode eRandomPedal)
        {
            ...
            else if (part == EInstrumentPart.DRUMS && eRandomPedal != ERandomMode.OFF && CDTXMania.ConfigIni.eNumOfLanes.Drums != DTXManiaNXa.EType.C)
            {
		...
		public void tRead( string strファイル名, bool bヘッダのみ, double db再生速度, int nBGMAdjust )
		{
			...
			if ( this.e種別 != EType.SMF )
			{
				try
				{
                    ...
                    //TimeSpan span;
                    StreamReader reader = CStreamReaderWithEncodingDetection.Create( strファイル名 );
