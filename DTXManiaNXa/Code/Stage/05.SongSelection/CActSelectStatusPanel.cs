        ...
        public void tSelectedSongChanged()
        {
            ...
            if ((c曲リストノード != null) && (cスコア != null))
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    {
                        this.n現在選択中の曲の最高ランク[i] = DTXManiaNXa.CScoreIni.tCalculateRank(0, cスコア.SongInformation.HighSkill[i]);
                    }
                    ...
                    for (int j = 0; j < 5; j++)
                    {
                        if (c曲リストノード.arScore[j] != null)
                        {
                            ...
                            else if (CDTXMania.ConfigIni.nSkillMode == 1)
                            {
                                ...
                                this.n現在選択中の曲の最高ランク難易度毎[j][i] = DTXManiaNXa.CScoreIni.tCalculateRank(0, c曲リストノード.arScore[j].SongInformation.HighSkill[i]);
                                dbCurrentSkillPointForAllDifficulty[j][i] = DTXManiaNXa.CScoreIni.tCalculateGameSkillFromPlayingSkill(
